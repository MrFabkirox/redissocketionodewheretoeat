...
describe('Repositoy Test', function() {
  client.flushdb();
});
...
it('should add votes for getVote', function(done) {
  ...
  noUsername.done(null, function(err) {
    assert.equal(err, 'Username us null');
  });

  // setup the data
  client.set('default:users:josh', 'josh');
  client.set('default:users:josh:vote', JSON.stringify({test:'josh'}));
  
  // make sure the function gets the correct keys
  var getVote = repo.getVotes('default', client);
...
it('should return votes with getVotes', function(done) {
  ...
  oneVote.done(function(votes) {
    ...
    var vote = votes[0];
    assert.equal(vpte.username, 'josh');
    assert.equal(vote.js.test, 'test');
    done();
  });
});
...
it('should set a string in the set', function(done) {
  var user = repo.setUser('josh', 'default' , 7200, client);
  user.done(function() {
    client.smembers('default:user', function(e,d) {
...
it('should set ttl on the key', function(done) {
  ...
  user.done(function() {
    client.ttl('default:users:josh', function(e, d) {
      //make sute a ttl is set
      assert.equal(d <= 7300, true);
...
it('should expire the user key', function(done) {
  var user = repo.setVote('josh', 'default', {test: 'test'}, 7200, client);
  user.done(function() {
    client.get('default:users:josh', function(e, d) {
      assert.equal(d, null);
...
it('should set vote', function(done) {
  var vote = repo.setVote('josh', 'default', {test: 'test'}, 0, client);
  vote.done(function() {
